...
import { useNavigation, useSharedContent } from "@/components/providers/SiteDataContext";
...
function SiteSearch() {
  ...
  return (
    <div>
      <form onSubmit={handleSubmit}>
        <div className="relative">
          <input
            ...
            aria-label="Search blackveteransproject.org"
            className="w-full px-5 py-4 pr-14 bg-transparent border border-white/20 text-white/80 placeholder:text-white/60 focus:outline-none focus:border-white/40 transition-colors"
          />
          <button
            type="submit"
            className="absolute right-4 top-1/2 -translate-y-1/2 w-10 h-10 flex items-center justify-center text-white/60 hover:text-white/80 transition-colors"
            aria-label="Search"
          >
      ...
      <p className="text-white/60 text-sm tracking-widest uppercase mt-3">
        Powered by Google
      </p>
    </div>
  );
...
// Default footer links (fallback when Sanity data is unavailable)
...
const defaultSocialLinks = [
  { platform: 'x', url: 'https://x.com/BlackVetProject', label: 'Follow us on X (formerly Twitter)' },
  { platform: 'instagram', url: 'https://www.instagram.com/BlackVeteransProject', label: 'Follow us on Instagram' },
  { platform: 'facebook', url: 'https://www.facebook.com/BlackVeteransProject/', label: 'Follow us on Facebook' },
  { platform: 'linkedin', url: 'https://www.linkedin.com/company/black-veterans-project/', label: 'Follow us on LinkedIn' },
  { platform: 'youtube', url: 'https://www.youtube.com/@blackveteransproject8335', label: 'Subscribe to our YouTube channel' },
  { platform: 'substack', url: 'https://blackveteransproject.substack.com', label: 'Read our Substack' },
];
...
function FooterColumn({ title, links }: { title: string; links: { name?: string; label?: string; href: string }[] }) {
  return (
    <div className="min-w-[140px]">
      ...
      <ul className="space-y-0">
        {links.map((link) => {
          ...
          return (
            <li key={linkText}>
              <FooterLink href={link.href}>{linkText}</FooterLink>
            </li>
          );
        })}
      </ul>
...
export function Footer() {
  // Get navigation and shared content from Sanity context
  const sanityNav = useNavigation();
  const sharedContent = useSharedContent();

  // Transform Sanity footer columns or use defaults
  const footerColumns = useMemo(() => {
    if (!sanityNav?.footerColumns) return defaultFooterColumns;

    return sanityNav.footerColumns.map(col => ({
      title: col.title,
  ...
  // Get social links from Sanity or use defaults
  const socialLinks = useMemo(() => {
    if (!sanityNav?.socialLinks) return defaultSocialLinks;

    return sanityNav.socialLinks.map(link => ({
      platform: link.platform,
      url: link.url,
      label: link.label || `Follow us on ${link.platform}`,
    }));
  }, [sanityNav]);

  // Copyright text from Sanity or default
  const copyrightText = sharedContent?.copyrightText || `© ${new Date().getFullYear()} Black Veterans Project. 501(c)(3)`;
          ...
          */}
          <div className="flex flex-wrap items-center justify-between gap-6">
            ...
            <div className="flex flex-wrap items-center gap-6">
              ...
              <div className="flex flex-col gap-1.5 text-white/70" style={{ fontSize: 'clamp(0.8125rem, 1.2vw, 0.9375rem)' }}>
            ...
            <div className="flex flex-wrap items-center gap-1">
              {socialLinks.map((link) => (
                <a
                  ...
                  className="w-10 h-10 flex items-center justify-center text-white/70 hover:text-bvp-gold transition-colors"
              ...
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center gap-2 text-[13px] font-gunterz font-medium uppercase tracking-[0.15em] text-white/70 hover:text-bvp-gold transition-colors"
              >
                Candid
