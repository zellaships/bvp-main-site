...
/**
 * API Route: /api/newsletter-signup-v2
 *
 * BVP Membership 3.0 Implementation:
 * - Newsletter signups are counted as Basic Members
 * - Submits to Action Network with appropriate tags
 * - Tags include "Substack Interest" for Zapier sync to Substack
 *
 * Tags generated:
 * - Basic Member (counts for membership)
 * - Newsletter (signup source)
 * - Substack Interest (triggers Zapier sync)
 */
...
function buildPayload(formData: NewsletterFormData) {
  ...
  // Per BVP Membership 3.0: Newsletter signups are Basic Members
  // Include Substack Interest tag for Zapier sync
  const tags = [
    'Basic Member',
    'Newsletter',
    'Website Signup',
  ...
  return {
    person: {
      ...
      custom_fields: {
        signup_source: 'Newsletter Strip V2',
        membership_tier: 'Basic Member',
        substack_sync: 'Yes',
      },
    },
    add_tags: tags,
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error('Newsletter signup v2 error:', error);
    return NextResponse.json(
